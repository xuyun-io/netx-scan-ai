...
POST /createTurn        → 202, status=IN_PROGRESS
POST /getTurn           → 轮询结果
POST /listRecords       → 查执行记录
POST /listArtifacts     → 查产物
```

### 2.2 通过 Task（任务模式）

```text
POST /createTask        → 202, status=PENDING
POST /getTask           → 查任务状态
POST /listRecords       → 查执行记录（按 taskId）
POST /listArtifacts     → 查产物（按 taskId）
```
...
status=COMPLETED
   │
   ▼
POST /listRecords (按 taskId)
POST /listArtifacts (按 taskId)
   │
   ▼
展示结果和产物
```
